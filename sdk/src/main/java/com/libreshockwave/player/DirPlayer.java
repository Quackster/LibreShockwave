 ...
 * Matches dirplayer-rs DirPlayer structure.
 */
public class DirPlayer {
    ...
    public void loadMovie(DirectorFile file) {
        ...
        // Initialize VM with player reference
    ...
    private void registerPlayerBuiltins() {
        ...
        vm.registerBuiltin("preload", (vmRef, args) -> {
            // Preload cast members - no-op for now
            return Datum.voidValue();
        });
    ...
    public void play() {
        if (state == PlayState.STOPPED) {
            // First time playing - trigger prepareMovie
    ...
    public void goToLabel(String label) {
        // Would look up label in score markers
        // For now, just stay on current frame
        System.out.println("[goToLabel] " + label);
    }

    /**
     * Execute one frame tick.
     * Called by the playback loop at tempo rate.
     */
    public void tick() {
        ...
        // Execute exitFrame handlers (may call go() to jump)
        int frameBefore = currentFrame;
        dispatchEvent(MovieEvent.EXIT_FRAME);

        // If exitFrame didn't change the frame, advance to next
    ...
    public void dispatchEvent(MovieEvent event) {
        ...
        // Find and execute handlers in scripts
        try {
            // First check frame scripts
    ...
    private void executeHandlerIfExists(String handlerName) {
        ...
        // Find the handler in any script
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("=== Director Player ===");
