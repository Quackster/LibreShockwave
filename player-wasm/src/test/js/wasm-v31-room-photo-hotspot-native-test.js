...
const outputDir = process.argv[3] || path.resolve(process.cwd(), 'frames_v31_room_photo_hotspot');
...
const exactPhotoPolls = Number(process.env.LS_V31_ROOM_PHOTO_EXACT_POLLS || 220);
const maxPhotoClickAttempts = Number(process.env.LS_V31_ROOM_PHOTO_MAX_CLICK_ATTEMPTS || 60);
...
const prePhotoClickSettleMs = Number(process.env.LS_V31_ROOM_PHOTO_PRE_CLICK_SETTLE_MS || 15000);
const prePhotoMoveClickX = Number(process.env.LS_V31_ROOM_PHOTO_PRE_MOVE_CLICK_X || 0);
const prePhotoMoveClickY = Number(process.env.LS_V31_ROOM_PHOTO_PRE_MOVE_CLICK_Y || 0);
const prePhotoMoveSettleMs = Number(process.env.LS_V31_ROOM_PHOTO_PRE_MOVE_SETTLE_MS || 3000);
const postPhotoClickMouseX = Number(process.env.LS_V31_ROOM_PHOTO_POST_CLICK_MOUSE_X || 0);
const postPhotoClickMouseY = Number(process.env.LS_V31_ROOM_PHOTO_POST_CLICK_MOUSE_Y || 0);
const postPhotoClickMouseSettleMs = Number(process.env.LS_V31_ROOM_PHOTO_POST_CLICK_MOUSE_SETTLE_MS || 300);
const runLegacyMovieRegressions = process.env.LS_V31_ROOM_PHOTO_SKIP_LEGACY_VISUALS !== '1';
const traceHandlers = (process.env.LS_TRACE_HANDLERS || '')
    .split(',')
    .map(name => name.trim())
    .filter(Boolean);
...
function collectByRules(sprites, textByChannel, mode) {
    ...
    const prioritized = base.sort((a, b) => {
        const aName = `${a.member} ${a.castName} ${a.dynName}`.toLowerCase();
        const bName = `${b.member} ${b.castName} ${b.dynName}`.toLowerCase();
        const aPhoto = mode === 'photo' && aName.includes('photo') ? 1 : 0;
        const bPhoto = mode === 'photo' && bName.includes('photo') ? 1 : 0;
        return (bPhoto - aPhoto)
            || ((textHints.get(b.channel) ? 1 : 0) - (textHints.get(a.channel) ? 1 : 0))
...
async function captureAndCompare(page, referenceUrl, expectedWidth, expectedHeight) {
    return page.evaluate(async ({ referenceUrl, expectedWidth, expectedHeight }) => {
        ...
        for (let p = 0; p < actualData.length; p += 4) {
            ...
            const da = Math.abs(actualData[p + 3] - referenceData[p + 3]);
            const delta = (dr + dg + db) / 3;
            total += delta;
            max = Math.max(max, delta);
            if (delta <= 8) matching++;
            if (delta > 64) bad++;
            if (dr !== 0 || dg !== 0 || db !== 0 || da !== 0) exactMismatch++;
        ...
        return {
            ...
            exactMismatch,
            exactMatchFraction: (pixels - exactMismatch) / pixels,
            maxDelta: max,
...
async function waitForExactMatch(page, referencePath, baseUrl, step, maxPolls = maxNativePolls, expectedWidth, expectedHeight) {
    let lastResult = null;
    for (let i = 0; i < maxPolls; i++) {
        await new Promise(resolve => setTimeout(resolve, pollMs));
        const result = await captureAndCompare(page, referenceUrl(baseUrl, referencePath), expectedWidth, expectedHeight);
        ...
        if (result.exactMismatch === 0) {
            return result;
        }
        if (i % 20 === 0) {
            const diag = await page.evaluate(() => ({ tick: window._testState.tick, frame: window._testState.frame, spriteCount: window.betaClientPlayer ? window.betaClientPlayer._lastSpriteCount || 0 : 0 }));
            console.log(`  [${step}] poll=${i} exactMismatch=${result.exactMismatch} exact=${(result.exactMatchFraction * 100).toFixed(2)}% mean=${result.meanDelta.toFixed(2)} bad=${(result.badFraction * 100).toFixed(2)}% sprites=${diag.spriteCount}`);
        }
    }
    throw new Error(`[${step}] Did not reach exact target reference. lastExactMismatch=${lastResult ? lastResult.exactMismatch : 'n/a'} lastMean=${lastResult ? lastResult.meanDelta : 'n/a'} lastBad=${lastResult ? lastResult.badFraction : 'n/a'}`);
}
...
async function saveComparison(page, refPath, outBase, result) {
    ...
    fs.writeFileSync(path.join(outputDir, `${outBase}_compare.txt`), JSON.stringify({
        ...
        exactMismatch: result.exactMismatch,
        exactMatchFraction: result.exactMatchFraction,
...
async function clickUntilMatch(page, candidates, targetRefPath, stepName, baseUrl, maxAttempts = 24, expectedWidth, expectedHeight) {
    ...
    for (let i = 0; i < candidates.length && i < maxAttempts; i++) {
        ...
        if (postPhotoClickMouseX > 0 && postPhotoClickMouseY > 0) {
            console.log(`  [${stepName}] moving mouse to neutral point (${postPhotoClickMouseX},${postPhotoClickMouseY}) before exact polling`);
            await page.mouse.move(postPhotoClickMouseX, postPhotoClickMouseY);
            await new Promise(resolve => setTimeout(resolve, postPhotoClickMouseSettleMs));
        }
        try {
            const match = await waitForExactMatch(page, targetRefPath, baseUrl, `${stepName}_attempt_${i}`, exactPhotoPolls, expectedWidth, expectedHeight);
            await saveComparison(page, targetRefPath, `${stepName}_match`, match);
            return match;
        } catch (err) {
            console.log(`  [${stepName}] attempt ${i} did not match: ${err.message}`);
            const last = await captureAndCompare(page, referenceUrl(baseUrl, targetRefPath), expectedWidth, expectedHeight);
            if (last && !last.error) {
                await saveComparison(page, targetRefPath, `${stepName}_attempt_${i}_last`, last);
            }
            ...
            fs.writeFileSync(path.join(outputDir, `${stepName}_attempt_${i}_visible_text.txt`), diagnostics.visibleText || '');
            fs.writeFileSync(path.join(outputDir, `${stepName}_attempt_${i}_window_sprites.txt`), diagnostics.windowSprites || '');
...
async function roomEnterTestV31ClickPhoto(page, baseUrl) {
    console.log('=== roomEnterTestV31ClickPhoto ===');
    await roomEnterTestV31(page, baseUrl);
    if (prePhotoMoveClickX > 0 && prePhotoMoveClickY > 0) {
        console.log(`Diagnostic pre-photo floor click at ${prePhotoMoveClickX},${prePhotoMoveClickY}; waiting ${prePhotoMoveSettleMs}ms...`);
        await clickPoint(page, { x: prePhotoMoveClickX, y: prePhotoMoveClickY, w: 1, h: 1 });
        await new Promise(resolve => setTimeout(resolve, prePhotoMoveSettleMs));
    }
    if (prePhotoClickSettleMs > 0) {
        console.log(`Waiting ${prePhotoClickSettleMs}ms for room avatar/object state to settle before clicking the photo...`);
        await new Promise(resolve => setTimeout(resolve, prePhotoClickSettleMs));
    }

    const beforeClickVsOpened = await captureAndCompare(page, referenceUrl(baseUrl, V31.clickedPath), 959, 540);
    if (!beforeClickVsOpened || beforeClickVsOpened.error) {
        throw new Error(`roomEnterTestV31ClickPhoto pre-click diff failed: ${beforeClickVsOpened ? beforeClickVsOpened.error : 'missing capture'}`);
    }
    await saveComparison(page, V31.clickedPath, 'roomEnterTestV31ClickPhoto_before_click_vs_opened_photo', beforeClickVsOpened);

    const photoCandidates = prependUniqueCandidates(await findAndDumpCandidates(page, 'photo'), MANUAL_PHOTO_CANDIDATES);
    const clickedTarget = await clickUntilMatch(page, photoCandidates, V31.clickedPath, 'roomEnterTestV31ClickPhoto_photo_click', baseUrl, maxPhotoClickAttempts, 959, 540);
    await saveComparison(page, V31.clickedPath, 'roomEnterTestV31ClickPhoto_opened_photo', clickedTarget);
    await saveComparison(page, V31.clickedPath, '04_clicked', clickedTarget);

    fs.copyFileSync(
        path.join(outputDir, 'roomEnterTestV31ClickPhoto_opened_photo.png'),
        path.join(outputDir, 'v31_photo_opened.png')
    );
    fs.writeFileSync(path.join(outputDir, 'roomEnterTestV31ClickPhoto_result.txt'), JSON.stringify({
        status: 'PASS',
        openedPhotoPath: path.join(outputDir, 'roomEnterTestV31ClickPhoto_opened_photo.png'),
        savedPhotoPath: path.join(outputDir, 'v31_photo_opened.png'),
        reference: V31.clickedPath,
        meanDelta: clickedTarget.meanDelta,
        badFraction: clickedTarget.badFraction,
        matchFraction: clickedTarget.matchFraction,
        maxDelta: clickedTarget.maxDelta,
        beforeClickMeanDelta: beforeClickVsOpened.meanDelta,
        beforeClickBadFraction: beforeClickVsOpened.badFraction,
    }, null, 2) + '\n');

    console.log('PASS: roomEnterTestV31ClickPhoto clicked wall photo and reached opened-photo reference.');
    return clickedTarget;
}

async function main() {
    console.log('=== WASM v31 room-photo hotspot regression ===');
    ...
    try {
        ...
        const clickedTarget = await roomEnterTestV31ClickPhoto(page, baseUrl);

        console.log('PASS: v31 room-photo hotspot regression reached expected reference.');
        fs.writeFileSync(path.join(outputDir, 'result.txt'), JSON.stringify({
            status: 'PASS',
            test: 'roomEnterTestV31ClickPhoto',
            openedPhotoPath: path.join(outputDir, 'roomEnterTestV31ClickPhoto_opened_photo.png'),
            savedPhotoPath: path.join(outputDir, 'v31_photo_opened.png'),
            reference: V31.clickedPath,
            meanDelta: clickedTarget.meanDelta,
