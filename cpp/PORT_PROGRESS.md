...
- `editor::gtk::GtkWorkbenchContentSpec` ports the GTK shell's active workbench render target, flattening the selected panel's title, body text, status text, action labels, and focus action names into a compact content snapshot with empty-state fallback.
...
- Current checkpoint commit message: `Wire C++ GTK panel row actions`
