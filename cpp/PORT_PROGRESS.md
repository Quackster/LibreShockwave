...
- Fixture validation: native `libreshockwave_probe` scans Director `.cct`, `.cst`, `.dcr`, `.dir`, and `.dxr` files through C++ load, editor member-scan, and score-grid paths while skipping extension-matched non-Director placeholder files and optionally reporting the active file during long scans.
