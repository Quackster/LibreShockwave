...
- Player render fixture coverage is verified with `libreshockwave_render_probe`. Full `/var/www/html` static render probing currently passes with 6,031 files rendered, 60 skipped, 0 failed, 430 external cast entries fetched/loaded from local sidecars, 80 collected sprites, 1 baked sprite, and 721 non-background pixels. Full `/var/www/html` score-frame scan coverage currently passes with 402 scanned frames, 366 frames with sprites, 29 frames with baked sprites, 29 nonblank frames, and 2 nonblank files: `dcr0910/loader.dcr` and `web-gallery/shockwave/detect_shockwave.dcr`. Bounded lifecycle render probing over `/var/www/html/dcr0910` currently passes with 64 files rendered, 0 skipped, 0 failed, 218 external cast entries fetched/loaded from local sidecars, and 1 captured startup script deadline error in `pellehyppy_script.cct` instead of hanging.
