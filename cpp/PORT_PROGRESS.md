...
- `lingo::Datum::media` now carries raw Director media bytes in C++, and bitmap `member.media` assignment routes those bytes through the shared `LSWI`/Director BITD decode path with the same script-modified copy and registration-point behavior as `importFileInto`.
...
- Common cast-member `number`, `memberNum`, `castLibNum`, `castLib`, `script`, `scriptText`, and `mediaReady` properties passed through the same CTest executable.
...
- Current checkpoint commit message: `Port C++ member common properties`
