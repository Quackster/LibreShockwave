...
### Lingo Decompiler Node Foundation
...
- `lingo::decompiler::LingoDecompiler` now ports Java-compatible script header/property/global emission, bytecode-only handler fallback formatting, handler line-to-bytecode mapping, AST-to-line mapping, and a bytecode-to-AST translator for literals including visible unknown byte-payload fallbacks, variables, arguments, locals, binary/unary expressions, assignments, calls, object/movie properties, v4 field/sprite/member property reads, chunk refs, put/delete chunk statements, sprite intersection/within expressions, object-call bracket/property/content rewrites, V4 object-call syntax, JavaScript comment payloads, object construction, basic `if`/`if/else` blocks, `repeat while` loops, `repeat with ... in/to/down to` loops, loop-control `next repeat`/`exit repeat` statements, `tell` blocks, and `case`/`otherwise` block reconstruction for simple, comma-separated, and computed expression labels.
...
- `editor::format::ChannelNames`, `PaletteDescriptions`, and `InstructionFormatter` port Java's score-channel labels, human-readable palette descriptions, and bytecode instruction display formatting for future editor panels.
- `editor::preview::PreviewFormatUtils` ports Java's shared text-preview formatting for member headers, palette dumps, and score appearance summaries/details.
...
- `editor::scanning::MemberResolver` and `FileProcessor` port Java's editor cast-member association and detail-building helpers for script, sound, palette, text/button, bitmap, shape, and film-loop members.
...
- `editor::preview::PreviewContextModel` ports Java's preview context data holder, status-label update, details-text update, and caret-reset behavior as GTK-neutral preview state.
- `editor::extraction::AssetExtractor` and `ExportHandlerModel` port Java's editor asset extraction for sound, script, text/button, palette, and generic member data plus single-file export safe-name/default-extension/filter/status decisions, with callback-based bitmap writing for a later GTK image export integration.
- `editor::debug` ports Java's debugger combo-box display items, instruction display state, call-target extraction, bytecode cell renderer breakpoint/current-line/navigable-call HTML presentation, stack/variable/watch/timeout/movie-property table models, and Objects-tab snapshot capture helpers as GTK-neutral row/value helpers.
- `editor::debug::DebugBrowserModels` ports Java's debugger handler navigation, script/handler filtering and silent selection behavior, bytecode instruction row construction, Lingo decompiler row mapping, breakpoint marker refresh, navigable call-target detection, and bytecode-debugger window header/status layout state for the future GTK debugger.
- `editor::debug::DebugCommandModels` ports Java's debugger toolbar layout/buttons, F-key action mapping, paused/running command enablement, and bytecode-list context menu breakpoint/navigation item visibility as GTK-neutral command metadata.
- `editor::debug::DebugInspectionModels` ports Java's datum-details dialog, detailed-stack window sizing/status/tab/text formatting, handler-details overview/bytecode/literal/property/global tab metadata, state-inspection tab order, ObjectsPanel layout/section/scroll metadata, watch panel buttons/prompts/selection/edit routing, and datum double-click detail titles as GTK-neutral inspection views.
...
- `editor::docking::DockingModels` ports Java's recursive docking layout state for center nodes, tab leaves, binary splits, edge docking, adjacent splits, tab splitting, undock collapse, default layout replay, saved-layout path helpers, compact JSON serialization, and serialized layout parse/replay as GTK-neutral panel layout data.
...
- `editor::EditorContextModels` ports Java's editor context property names/events, file/frame/playback lifecycle transitions, forced frame repaint events, playback timer-delay calculation, local HTTP root detection, and breakpoint preference-key sanitizing as GTK-neutral state.
- `editor::EditorFrameModels` ports Java's editor frame panel creation order, floating visibility/default bounds, Window-menu visibility snapshots, show/toggle/reset/float panel behavior, open-file dialog metadata, and docking interaction as GTK-neutral frame state.
...
- `editor::castbrowser::CastBrowserModels` ports Java's cast-window selector entries, type/search filtering, grid/list row text, wrapping grid layout geometry, popup-list selection preservation, selection retention, context action enablement, editor-target routing, bitmap thumbnail placement math, and placeholder/checkerboard thumbnail presentation metadata as GTK-neutral cast browser state.
- `editor::score::ScoreViewModels` ports Java's custom score-panel preferred sizing, grid/frame-label/playback-head geometry, channel header rows, marker triangle/text placement, score-cell tooltip/background presentation, load/frame status strings, and special-channel/member-type color routing as GTK-neutral view data.
- `editor::stage::StageViewModels` ports Java's stage-window size fallback, centered canvas/border geometry, no-movie/debug overlay state, mouse coordinate translation, key-character normalization, control/meta key filtering, Director key-code dispatch data, and stage title formatting as GTK-neutral state.
...
- `editor::gtk::EditorGtkShellModel` ports GTK-facing action names, detailed action identifiers, toolbar item bindings, stateful panel toggle actions, and panel list row presentation into a compiled/tested adapter used by the optional GTK4 scaffold.
- `editor::gtk::GtkMenuSpec` ports GTK-facing menu tree metadata, including top-level mnemonics, command/check/separator/submenu item kinds, detailed action names, accelerator strings, submenu children, and Window-menu panel checked state from the current frame model.
- `editor::gtk::EditorGtkShellState` ports GTK shell action activation behavior for panel show/hide toggles, reset-layout state restoration, disabled-action reporting, unknown-action reporting, and exit requests while keeping the optional GTK callbacks backed by compiled/tested C++ state.
- `editor::gtk::GtkShellViewState` ports the optional GTK shell's refresh snapshot for window title/size/background, stage placeholder text, status messages, action states, toolbar items, and panel sidebar rows so action callbacks can rebuild visible shell state from compiled/tested C++ data.
- `editor::gtk::EditorGtkShellState` now also owns GTK shell file lifecycle state: Open requests return file-dialog metadata, accepted file paths update the editor context/title/stage placeholder/status, Close clears the context with Java-style file events, and the optional GTK scaffold can seed the shell from a command-line movie path.
- `editor::gtk::GtkOpenFileDialogPresentation` ports GTK-native file chooser metadata for Director movie opens, including filter labels, extensions, glob patterns, current folder handoff, Open/Cancel labels, modal state, single-selection state, and existing-file requirements.
- `editor::gtk::GtkOpenFileDialogButtonSpec` now exposes stable GTK action names and detailed action names for Open and Cancel file-dialog buttons, and `openFileDialogActionSpecs` derives installable action metadata from those button specs.
- `editor::gtk::EditorGtkShellState` can now route Open/Cancel file-dialog action names back into tested accept/cancel behavior, including missing-selection and unknown-action status handling for future native GTK file chooser callbacks.
- `editor::gtk::EditorGtkShellState` ports GTK shell playback command state for Play, Stop, Rewind, Step Forward, and Step Backward actions, including Java-style context events, status messages, no-movie guards, current-frame snapshots, and dynamic toolbar frame labels.
- `editor::gtk::GtkShellDialogRequest` ports GTK shell dialog/panel request metadata for External Parameters, Trace Handler, About, and Detailed Stack commands, reusing the external-params table model and detailed-stack view model where available.
- `editor::gtk::GtkShellDialogPresentation` ports GTK-native dialog rendering metadata for External Parameters, Trace Handler, About, and Detailed Stack requests, including body/input text, table/detail-view flags, and Apply/Cancel/OK/Close button labels.
- `editor::gtk::GtkShellDialogButtonSpec` now exposes stable GTK action names and detailed action names for Apply, Cancel, OK, and Close dialog buttons, and `dialogActionSpecs` derives installable action metadata from those button specs.
- `editor::gtk::EditorGtkShellState` ports GTK shell dialog result state for External Parameters and Trace Handler requests, including Java-compatible parameter table sanitization, trace-handler input parsing, request prefill, change detection, and status updates.
- `editor::gtk::EditorGtkShellState` now also ports GTK shell open-file accept/cancel and dialog-cancel result handoff, keeping cancelled dialogs non-mutating while accepted file selections reuse the tested editor context/preference lifecycle.
- `editor::gtk::EditorGtkShellState` ports Java's GTK shell preference handoff for accepted movie opens: last-open directory and recents update from selected paths, saved per-movie external parameters are loaded and persisted, `/htdocs/` movies receive the default Habbo parameter preset when no saved params exist, and per-player trace/external-param state clears on close/open transitions.
- `editor::gtk::GtkWorkbenchPanelSpec` ports the optional GTK shell's visible workbench panel snapshot, including panel kind/layout, dynamic Stage and Score titles/statuses, Message console text, Tool Palette tools, Paint/Text/Field/Sound/Color placeholder states, and action labels derived from the existing GTK-neutral panel models.
- `editor::gtk::EditorGtkShellState` ports GTK workbench panel activation/focus state, including available-panel selection, docked-tab selection, selected workbench refresh metadata, and unavailable-panel reporting for hidden or iconified panels.
- `editor::gtk::GtkWorkbenchLayoutSpec` ports the GTK shell's active workbench layout snapshot, giving the GTK scaffold a deterministic active panel, visible panel list, empty-state text, and single-selection behavior when panel actions dock/show panels.
- `editor::gtk::EditorGtkShellModel` now emits stable GTK workbench focus action names for visible panels, and `EditorGtkShellState` can resolve those action names back into tested panel activation results for future GTK sidebar/tab callbacks.
- `editor::gtk::GtkWorkbenchFocusActionSpec` ports the GTK shell's installable workbench focus action state, including per-panel detailed action names, enabled state for visible/non-iconified panels, and active state for the selected workbench panel.
- `editor::gtk::GtkPanelRowSpec` now carries GTK sidebar binding metadata for each panel row, including Window-menu toggle action names, workbench focus action names, detailed GTK action names, and focus-enabled state.
- `editor::gtk::GtkWorkbenchContentSpec` ports the GTK shell's active workbench render target, flattening the selected panel's title, body text, status text, action labels, disabled action specs, and focus action names into a compact content snapshot with empty-state fallback.
- `editor::gtk::GtkWorkbenchTabSpec` ports GTK workbench tab metadata for visible panels, including dynamic titles, active-panel state, focus action bindings, float action bindings, and Window-menu toggle action bindings for future native GTK tab controls.
- `editor::gtk::GtkWorkbenchTabSpec` now also carries GTK tab-control presentation metadata for active/select tooltips and Float/Hide labels/tooltips, and the optional GTK scaffold consumes those tested fields instead of hardcoded tab-control text.
- `editor::gtk::EditorGtkShellState` ports GTK workbench tab float actions, exposing Java-style undock-and-show behavior through stable action names and tested refresh metadata for future native GTK tab close/float controls.
- `editor::gtk::EditorGtkShellModel` now includes workbench focus and float actions in the installable GTK action snapshot, and `EditorGtkShellState::activateAction` routes those action names through the same tested callback path as menu and Window actions.
- `editor::gtk::GtkStartScreenRequest` ports the GTK shell's launch-without-movie handoff, exposing title/subtitle text, recent-project rows with existence state, open-file dialog metadata, disabled Create New Movie state, and tested recent-project open/failure behavior.
- `editor::gtk::GtkStartScreenPresentation` ports Java's start-screen GTK presentation metadata, including Recent Projects labels, Open Movie/Create New Movie button state, empty-recents fallback, and existing/missing recent-project row rendering state.
- `editor::gtk::GtkRecentProjectRowSpec` now exposes stable per-row GTK action names and detailed action names, and `EditorGtkShellState` can route those action names back into tested recent-project open/failure behavior for future native GTK start-screen buttons.
- `editor::gtk::EditorGtkShellModel::startScreenActionSpecs` now derives installable GTK action specs for Open Movie, disabled Create New Movie, and per-row recent-project actions from the tested start-screen presentation metadata.
- `editor::gtk::GtkActionSpec` now carries GTK-formatted accelerator strings for command and Window-menu actions, preserving Java menu shortcuts including control/alt/shift combinations and special keys for future native GTK accelerator wiring.
- `editor::gtk::GtkActionAcceleratorSpec` ports the GTK accelerator-install list derived from action specs, and the optional GTK scaffold now applies those shortcuts through `gtk_application_set_accels_for_action` when GTK4 is available.
...
- `editor::EditorFramePanelModel` and `editor::gtk::EditorGtkShellState` now support moving visible undocked panes by drag offsets, and the optional GTK scaffold updates floating pane bounds when a drag ends outside snap zones.
- `editor::gtk::EditorGtkShellState` now treats non-snap drags on docked panes as detach-and-move operations, so docked panes can be dragged back into the floating pane area.
...
- `bitmap::Drawing` now owns image matte/mask creation for native-alpha sources, RGB/indexed flood-fill matte extraction, and direct grayscale mask generation.
- `bitmap::Drawing` ports Java's `applyFloodFillTransparency` and `applyMatteToRegion` helpers for edge-connected matte removal.
- `bitmap::Drawing` ports Java's pixel-level `applyInk`, alpha blending, and alpha-combination helpers for copyPixels ink modes.
- `bitmap::Drawing` ports Java's coordinate-based `copyPixels` rectangle traversal, mask gating, MATTE mask-destination conversion, and background-transparent preprocessing/keying helpers.
- `bitmap::Drawing` now owns bitmap quad transforms for `copyPixels` quad destinations, including palette metadata and palette-index propagation.
- VM image `draw` methods now share these bitmap drawing primitives instead of carrying duplicate local implementations.
- VM image `createMatte` and `createMask` methods now dispatch through the reusable bitmap drawing helpers.
- VM `copyPixels` MATTE source preparation now uses the reusable bitmap drawing flood-fill transparency helper.
- VM `copyPixels` per-pixel ink blending now dispatches through the reusable bitmap drawing `applyInk` helper.
- VM `copyPixels` mask checks and background-transparent preprocessing now share reusable bitmap drawing helpers.
- VM-specific `copyPixels` scaling, palette-index refresh, color-remap, and Lingo prop-list handling remain implemented in the VM receiver path.
...
- `CastLibManager::resolvePaletteById` now ports Java's CastLibProvider exact palette-ID lookup across requested cast libraries, the main movie file, built-in palettes, dynamic palette members, and already-loaded fallback casts; imported Director BITD media now uses that ID path before member-number palette fallback.
...
- `CastLibManager::resolvePaletteByMember` exposes cast-aware authored and dynamic palette member resolution for player/runtime callers, `resolvePaletteById` covers Java-style exact palette-ID provider lookups, and `resolvePaletteByName` resolves named runtime palette members; broader dynamic member resolver coverage remains deferred to later Player integration slices.
...
- Player now refreshes installed debug-controller global snapshots before C++ `stepFrame`/`tick` VM execution and resets the controller during `shutdown`, matching Java's debugger runtime handoff and blocked-thread release hook for the C++ frame entry points.
...
- Player-owned sound playback now resolves cast-member audio through `CastLibManager`, so loaded external-cast sound members follow Java's cast-library source lookup instead of only the main movie file.
...
- `render::pipeline::SpriteBaker` ports ordered sprite bake steps, the explicit Java-style unsupported pass-through step, tick counting for batch bakes, custom bake-step registration, immutable `RenderSprite` baked-bitmap attachment, and text/button size replacement when baked output dimensions differ from score dimensions.
...
- The SimpleTextRenderer XMED path now ports Java's per-span bitmap rendering for mixed span fonts, colors, underline runs, paragraph/word-wrap layout, authored font-candidate fallback, and styled underline handling after single-font fast-path renders while still supporting the default `TextRenderer` delegation path; `DirectorFile` can resolve file-backed XMED chunks for text-Xtra members through KEY* ownership.
...
- `lingo::vm::parse::LingoExpressionParser` ports Java's strict expression-string helper for integers, floats, symbols, quoted strings, nested lists/proplists, and VM-backed zero-argument handler/global lookup without partial-expression fallback.
...
- `lingo::vm::dispatch::ScriptInstanceMethodDispatcher` ports Java script-instance receiver dispatch through the existing C++ handler/property/member-registry surface, covering property/list-like accessors, ancestor-aware setters, exact-key local `addProp`/`deleteProp`, nested list/proplist mutation, counts, `ilk`, deferred numeric `closeThread`, provider-owned authored handler lookup without current-handler fallback, member-registry prefill/fallbacks, and property fallback reads.
...
- Mutable ChunkRef creation and char/word/item/line deletion now use a dedicated C++ datum, including movie item-delimiter routing for item chunk refs.
...
- Editor app-shell preferences/start-screen/external-param table models, editor context lifecycle/playback/preference-key state models, editor frame panel visibility/toggle/reset/float models, panel catalog/default-layout metadata, panel window content/state/layout metadata including media panel toolbars/statuses and message/tool/color panel sizing, GTK shell action/toolbar/panel/menu adapter metadata, GTK toolbar icon/enabled metadata, GTK open-file button/action metadata and routing, GTK start-screen recent/action-spec metadata, GTK dialog button/action metadata, GTK action accelerator metadata and native accelerator install specs, GTK action activation state behavior, GTK shell view-state snapshots, GTK shell file lifecycle state, GTK shell playback command state, GTK shell dialog request/result/cancel/presentation state, GTK shell open-file result/presentation state, GTK shell preference handoff/start-screen request/presentation handoff, GTK workbench panel snapshots/activation/layout/focus actions/focus action state/content state/content action specs/tab state/tab control metadata/tab float actions/installable action routing, GTK panel sidebar row bindings and primary row action routing, GTK scaffold menu/tab/content/toolbar refresh wiring, renderer row presentation metadata, menu/toolbar shell action metadata, debugger command/shortcut/context-menu metadata, debugger inspection/detail/handler-detail/state-tab/watch/object metadata including ObjectsPanel layout and scroll sizing plus watch double-click edit requests, debugger bytecode cell/window-header presentation metadata, debugger Objects-tab timeout/global/movie-property snapshot helpers, cast browser selector/filter/selection/action/wrap-layout/thumbnail presentation models, score view geometry/presentation models, stage canvas/input/title models, channel-name formatting, palette descriptions, bytecode instruction formatting, preview text formatting, preview context status/details state, palette/script/sound/text/generic preview generation, sound/script/text/palette/generic asset extraction, single-file export filename/filter/status helpers, property-inspector row models, script editor dropdown/document/highlight models, editor audio playback/backend models, docking layout/save-load replay models, score color lookup, model record display strings, selection manager notifications, playback-head clamping, score model color storage, score appearance formatting, score-grid/frame-label/frame-script display helpers, member association/detail scanning, Lingo keyword tables, and Lingo tokenizer behavior passed through the same CTest executable.
...
- GTK floating pane movement coverage now verifies non-snap drags update visible undocked pane bounds while hidden and docked panes do not move.
- GTK docked-pane drag coverage now verifies a non-snap drag detaches the pane from the dock layout, moves it into `floatingPanels`, and refreshes dock/action state.
...
- `d9cf18da Port C++ editor score data helpers`
- `762ed8b1 Port C++ editor formatting helpers`
...
- `7012c581 Port C++ editor shell action models`
- `bb128e8a Port C++ editor cast browser models`
- `53d4f605 Port C++ editor score view models`
- `688eeff7 Port C++ editor stage view models`
- `ef41a5bb Port C++ editor panel catalog`
- `fdc791ab Port C++ editor renderer models`
- `cbbb7987 Port C++ editor debug command models`
- `c291dfff Port C++ editor debug inspection models`
- `db34ad5b Port C++ editor debug state inspection models`
- `0a23bfa3 Port C++ editor bytecode cell presentation`
- `011a005c Port C++ editor panel window models`
- `1ff8c726 Port C++ editor media panel models`
- `8186b2c6 Port C++ editor context lifecycle models`
- `daa8d205 Port C++ editor export handler models`
- `7c98f7e4 Port C++ editor docking layout persistence`
- `e01ebbb1 Port C++ editor frame panel models`
- `f163c359 Port C++ editor preview context model`
- `21d40efa Port C++ cast browser layout models`
- `9f338f99 Port C++ editor debug handler details models`
- `555146e5 Port C++ cast browser thumbnail presentation`
- `bffff91e Port C++ bytecode debugger window header`
...
- `da838f95 Add optional C++ GTK editor scaffold`
- `03839f72 Port C++ GTK editor shell models`
- `9df21666 Port C++ GTK shell action state`
- `9c6862fa Port C++ GTK shell view state`
- `6d7855cf Port C++ GTK shell file lifecycle`
- `4ed6dd51 Port C++ GTK shell playback state`
- `8c654f11 Port C++ GTK shell dialog requests`
- `b0fd3b36 Port C++ GTK shell dialog results`
- `299baf38 Port C++ GTK shell preference handoff`
- `b6d518b1 Port C++ GTK workbench panel snapshots`
- `4476b739 Port C++ GTK workbench panel activation`
- `062461d9 Port C++ GTK workbench layout state`
- `98368a18 Port C++ GTK workbench focus actions`
- `b9f5890c Port C++ GTK workbench focus action state`
- `374d7d7b Port C++ GTK panel sidebar bindings`
- `9e84b937 Port C++ GTK workbench content state`
- `8c50eed9 Port C++ GTK workbench tab state`
- `006e8ec6 Port C++ GTK workbench tab float actions`
- `bede11aa Port C++ GTK workbench installable actions`
- `c29fa073 Port C++ GTK start screen handoff`
- `be8f825a Port C++ GTK dialog result handoff`
- `a4af7453 Port C++ GTK action accelerators`
- `54ba5c49 Wire C++ GTK action accelerators`
- `e65d2daf Wire C++ GTK panel row actions`
- `a0f9feb0 Wire C++ GTK workbench content view`
- `fc58eead Wire C++ GTK workbench tabs`
- `8fd5f437 Port C++ GTK dialog presentation metadata`
- `f60dfd90 Port C++ GTK open-file presentation metadata`
- `3f28275f Port C++ GTK start screen presentation`
- `538575a6 Wire C++ GTK tab control metadata`
- `d5b04b7b Port C++ GTK menu specs`
- `791243bd Wire C++ GTK menu specs`
- `1fe00733 Wire C++ GTK toolbar icon metadata`
- `d5b764fe Port C++ GTK recent project actions`
- `62bd10a6 Port C++ GTK start screen action specs`
