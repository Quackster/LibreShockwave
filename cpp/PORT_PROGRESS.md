...
- Image receiver dispatch now supports Java-compatible `fill`, `setAlpha`, `duplicate`, `crop`, `trimWhiteSpace`, `getAt`, `getPixel`, and `setPixel` methods over C++ bitmap refs.
...
- Lingo image object-method fill, setAlpha, duplicate, crop, trimWhiteSpace, getAt, getPixel, setPixel, and null-image fallback tests passed through the same CTest executable.
...
- Current checkpoint commit message: `Port C++ image alpha method foundation`
